...
const log = createLogger('Arknights code');
...
if (process.env.MAKO_SERVER_MODE === '1') {
  ...
  if (fs.existsSync(serverPath)) {
    ...
    console.error('[Arknights code] Server mode: server.js not found');
    process.exit(1);
  }
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    const env = {
      ...
      // 当前对话干员（server.js 据此注入对应 operators/<name>.md 人设）
      ARKCODE_ACTIVE_OPERATOR: readActiveOperator(),
      // 用户数据目录（server.js 据此存放本地音乐/收藏等）
      ARKCODE_USERDATA: app.getPath('userData'),
      NO_COLOR: '1',
      NODE_NO_WARNINGS: '1',
    };
...
// 读取当前激活干员（operators/active.json，缺省 阿米娅）
function readActiveOperator() {
  try {
    const f = path.join(APP_DIR, 'operators', 'active.json');
    if (fs.existsSync(f)) {
      const d = JSON.parse(fs.readFileSync(f, 'utf8'));
      if (d && d.active) return d.active;
    }
  } catch {}
  return '阿米娅';
}
...
// 持久化激活干员
function writeActiveOperator(name) {
  try {
    const dir = path.join(APP_DIR, 'operators');
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'active.json'), JSON.stringify({ active: name }, null, 2), 'utf8');
    return true;
  } catch (e) {
    log(`writeActiveOperator error: ${e.message}`);
    return false;
  }
}
...
function restoreUserConfig() {
  ...
  // operators/active.json — 安装目录缺失时从备份恢复（保留用户选择的干员）
  const activeSrc = path.join(backupDir, 'operators', 'active.json');
  const activeDst = path.join(APP_DIR, 'operators', 'active.json');
  if (!fs.existsSync(activeDst) && fs.existsSync(activeSrc)) {
    try {
      if (!fs.existsSync(path.dirname(activeDst))) fs.mkdirSync(path.dirname(activeDst), { recursive: true });
      fs.copyFileSync(activeSrc, activeDst);
      log('Config: restored operators/active.json from backup');
    } catch (e) {
      log(`Config: failed to restore operators/active.json: ${e.message}`);
    }
  }
}
...
function backupUserConfig() {
  try {
    ...
    // operators/active.json（保留用户选择的干员）
    const activeSrc = path.join(APP_DIR, 'operators', 'active.json');
    if (fs.existsSync(activeSrc)) {
      const activeDst = path.join(backupDir, 'operators', 'active.json');
      if (!fs.existsSync(path.dirname(activeDst))) fs.mkdirSync(path.dirname(activeDst), { recursive: true });
      fs.copyFileSync(activeSrc, activeDst);
    }
...
function createFallbackWizard() {
  ...
  if (!fs.existsSync(wizardPath)) {
    dialog.showErrorBox('启动失败', '找不到向导页面文件，请重新安装 Arknights code。');
    app.quit();
    return;
  }
...
ipcMain.handle('read-persona', async () => {
  const active = readActiveOperator();
  const personaFile = path.join(APP_DIR, 'operators', `${active}.md`);
  const loreFile = path.join(APP_DIR, 'operators', `${active}_lore.md`);
  try {
    const result = { operator: active };
    if (fs.existsSync(personaFile)) {
      result.persona = fs.readFileSync(personaFile, 'utf8');
    }
    if (fs.existsSync(loreFile)) {
      result.lore = fs.readFileSync(loreFile, 'utf8');
    }
...
// ─── 激活干员读写（供前端在 file:// 后备模式下切换）────
ipcMain.handle('get-active-operator', async () => {
  return { ok: true, active: readActiveOperator() };
});

ipcMain.handle('set-active-operator', async (_event, name) => {
  if (!name) return { ok: false, error: '缺少干员名' };
  const ok = writeActiveOperator(name);
  return { ok, active: name };
});

// ─── 导入本地音乐（复制进用户数据目录，返回可播放列表）────
// 已知 PRTS 文件名 → 友好标题映射（用户从 D:\新建文件夹 导入的 6 首等）
const MUSIC_NAME_MAP = {
  'm_sys_title_combine (1)': 'Rhodes Island（系统登录界面）',
  'm_sys_title_combine': 'Rhodes Island（2nd Edition）',
  'm_sys_void_combine (1)': '生命流',
  'm_sys_void_combine': '生命流',
  'm_sys_science_combine (1)': '泛用型自动化解决方案0.3.2.9f2',
  'm_sys_science_combine': '泛用型自动化解决方案0.3.2.9f2',
  'm_sys_shop_combine (1)': '休憩',
  'm_sys_shop_combine': '休憩',
  'm_bat_victory_combine': '终局抵抗者',
  'm_bat_failed_combine': '血液',
};

function musicDataDir() {
  const dir = path.join(app.getPath('userData'), 'arkcode-music');
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  return dir;
}
...
ipcMain.handle('import-music', async () => {
  try {
    ...
    if (result.canceled || !result.filePaths || result.filePaths.length === 0) {
      return { ok: true, canceled: true, tracks: [] };
    }
    const dir = musicDataDir();
    const imported = [];
    for (const src of result.filePaths) {
      const base = path.basename(src);
      const dest = path.join(dir, base);
      try { fs.copyFileSync(src, dest); } catch (e) { log(`import-music copy err: ${e.message}`); continue; }
      const stem = base.replace(/\.[^.]+$/, '');
      const title = MUSIC_NAME_MAP[stem] || MUSIC_NAME_MAP[stem.replace(/\s*\(\d+\)$/, '')] || stem;
      imported.push({ name: base, title });
    }
    log(`import-music: imported ${imported.length} file(s)`);
    return { ok: true, canceled: false, tracks: imported };
  } catch (e) {
    log(`import-music error: ${e.message}`);
    return { ok: false, error: e.message };
  }
});
