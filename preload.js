...
contextBridge.exposeInMainWorld('makoElectron', {
  ...
  // ─── 干员人设 ──────────────────────────────────────────
  // 读取干员人设 markdown 文件（operators/<激活干员>.md）
  readPersona: () => ipcRenderer.invoke('read-persona'),

  // 保存干员人设 markdown 文件
  writePersona: (data) => ipcRenderer.invoke('write-persona', data),

  // 读取/设置当前激活干员（file:// 后备模式）
  getActiveOperator: () => ipcRenderer.invoke('get-active-operator'),
  setActiveOperator: (name) => ipcRenderer.invoke('set-active-operator', name),

  // 导入本地音乐文件（Electron 文件对话框，复制进用户数据目录）
