; ============================================================
; Arknights code NSIS 自定义安装脚本
; 阿米娅风格的安装向导 + 环境依赖检测 + 自动子目录
; ============================================================

!include "FileFunc.nsh"

; --- 设置默认安装路径 + 始终追加 Arknights code 子目录 ---
; customInit 运行在 .onInit 中（安装器初始化时，页面弹出前）
; 设置默认 $INSTDIR 包含 Arknights code；同时通过函数钩子保证目录页离开时也修正

!macro customInit
  ; 设置默认安装目录包含 Arknights code
  ${GetFileName} $INSTDIR $0
  ${If} $0 != "Arknights code"
    StrCpy $INSTDIR "$INSTDIR\Arknights code"
  ${EndIf}
!macroend
...
!macro customInstall
  ...
  ${If} $R0 != ""
    MessageBox MB_OK|MB_ICONINFORMATION "博士！阿米娅发现有些东西还没装好：$R0$\r$\n$\r$\n不过别担心~第一次打开 Arknights code 的时候，阿米娅会一步步教博士安装的。$\r$\n$\r$\n现在先放心点确定吧。"
  ${EndIf}
!macroend
